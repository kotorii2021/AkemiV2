...
const { state, saveState } = await useSingleFileAuthState(qrcode)
    ...
    const aqua = aquaConnect({
        ...
        browser: ['AQUA BOT SUPREMACY','LICHT SAN','V 3.0'],
        auth: state
})
...
//Apague as 2 Barras Acima
//Caso Queira Que o Bot leia as Mensagens e Veja Status
...
require('./Aqua-Menus/menu')
	require('./CONFIGS DIVINA.js')
	require('./AQUA SUPREMACY.js')
	require('./index')
  nocache('./Aqua-Menus/menu', Modulo => console.log(chalk.greenBright('[ WHATSAPP BOT ]  ') + time + chalk.cyanBright(` Modulo "${Modulo}" atualizado`)))
	nocache('./CONFIGS DIVINA.js', Modulo => console.log(chalk.greenBright('[ WHATSAPP BOT ]  ') + time + chalk.cyanBright(`Modulo "${Modulo}" atualizado!`)))
	nocache('./AQUA SUPREMACY.js', Modulo => console.log(chalk.greenBright('[ WHATSAPP BOT ]  ') + time + chalk.cyanBright(`Modulo "${Modulo}" atualizado!`)))
	nocache('./index', Modulo => console.log(chalk.greenBright('[ WHATSAPP BOT ]  ') + time + chalk.cyanBright(`Modulo "${Modulo}" atualizado!`)))
	
//===================================== 

tchaugay = global.tchau

bemvindogay = global.bv
...
Lft = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/d460e086f9f9bf6b04e17.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
  ...
  ╭━━━━━◉                                       ◉━━━━━╮
       ...
       ╚┉${emoji1}┉═══『💧』═══┉${emoji1}┉╝    
╰━━━━━◉                                       ◉━━━━━╯
ㅤㅤི⋮ ྀ${emoji2}⏝ ི⋮ ྀ  ${emoji1} ི⋮ ྀ⏝${emoji2}⋮ ྀ

┠➥ [💦] ${bemvindogay}
╰─╼━━━══━━━≺💧≻━━━══━━━╾┄╯`

let buttons = [
{buttonId: `${prefix}login`, buttonText: {displayText: '[💦] CRIAR LOGIN [💦]'}, type: 1},
...
let buttons = [
{buttonId: `${prefix}reviver`, buttonText: {displayText: '[😥] VOLTA LOGO [😥]'}, type: 1},
